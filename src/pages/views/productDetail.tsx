...
export default function ProductPage({ product }: ProductPageProps) {
  ...
  const colors = [
    ...
    const keyFeatures = Array.isArray(specJson["Key Features"]) ? specJson["Key Features"] : [];
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <div className="bg-white mt-16">
                ...
                <div className="flex items-center gap-2 text-sm">
                    ...
                    <Link href="/products" className="text-gray-500 hover:text-blue-600 cursor-pointer transition">Product</Link>
                    <span className="text-gray-400">/</span>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
                        ...
                        {images?.length > 1 && (
                            ...
                            {images?.map((img, idx) => (
                                ...
                                >
                                <Image width={500} height={500} src={img} alt={`Thumbnail ${idx + 1}`} className="w-full h-full object-cover" />
                                </button>
                            ))}
                            </div>
                        )}
                        ...
                        <div>
                            ...
                            <span className="text-gray-600">{product.rating} ({product.sales} sold)</span>
                            </div>
                            <div className="flex items-baseline gap-4">
                            <span className="text-4xl font-bold text-gray-900">
                                {product.discount && product.discount > 0 ? formatPrice(product.price - (product.price * product.discount / 100)) : formatPrice(product.price)}
                            </span>
                            {product.discount && product.discount > 0 ? (
                                ...
                                <span className="px-3 py-1 bg-red-100 text-red-600 rounded-full text-sm font-semibold">{product.discount}% OFF</span>
                                </>
                            ) : null}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {product.stock && (
                                ...
                                {product.stock > 10 ? 'In Stock' : `Only ${product.stock} items left`}
                                </span>
                            )}
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4">
                            ...
                            className="flex-1 bg-gray-900 text-white py-4 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-gray-800 transition-all shadow-lg hover:shadow-xl disabled:bg-gray-400 disabled:cursor-not-allowed"
                            >
                    ...
                    <div className="border-t bg-gray-50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="bg-white rounded-2xl p-6 shadow-md">
                                    ...
                                    <div className="space-y-4">
                                        {specs.map((spec, idx) => (
                                        <div key={idx} className="flex flex-col sm:flex-row sm:justify-between gap-2 py-3 border-b border-gray-100 last:border-0">
                                            <span className="font-semibold text-gray-700 flex-shrink-0">{spec.label}</span>
                                            <span className="text-gray-600 text-left sm:text-right break-words">{spec.value}</span>
                                        </div>
                                        ))}
                                    </div>
                                </div>
