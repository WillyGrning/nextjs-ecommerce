...
export default function ProductPage() {
  ...
  type SortOptionKey = Exclude<sortKey, "">;
  ...
  useEffect(() => {
    async function fetchCategories() {
      ...
      const mapped: CategoryWithCount[] = data.map((cat) => ({
        id: cat.id,
        name: cat.name,
        slug: cat.slug,
  ...
  const categoryList: CategoryWithCount[] = [
    {
      id: "all",
      name: "All Products",
      slug: "all",
      product_count: allProductCount,
    },
    ...categories,
  ];
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      if (selectedPriceRange.length > 0) {
        const mins = selectedPriceRange
          .map((r) => priceOptions[r].min)
          .filter((v): v is number => v !== undefined);

        const maxs = selectedPriceRange
          .map((r) => priceOptions[r].max)
          .filter((v): v is number => v !== undefined);

        if (mins.length) query = query.gte("price", Math.min(...mins));
        if (maxs.length) query = query.lte("price", Math.max(...maxs));
      }

      // rating filter
      if (selectedRating.length > 0) {
        query = query.gte("rating", Math.min(...selectedRating));
      }
            ...
            <div className="bg-white rounded-xl p-6 sticky top-8">
                ...
                <div className="space-y-2">
                  {categoryList.map((cat) => (
                    <button
                      ...
                      className={`w-full text-left px-4 cursor-pointer py-2.5 rounded-lg flex justify-between ${
              ...
              <div className="mb-8 pb-8 border-b">
                ...
                <div className="space-y-3">
                  {(
                    ["under-500", "500-1000", "over-1000"] as priceRangeKey[]
                  ).map((range) => (
                    <label
                      key={range}
