...
import { ShoppingCart, X, Plus, Minus, Trash2, ArrowRight, Tag, Truck, Shield, Lock } from 'lucide-react';
...
type CartItemWithProduct = CartItem & {
  products: Pick<ProductRow, "id" | "name" | "image" | "price" | "stock" | "discount"> | null;
};
...
export default function CartPage() {
   ...
   const handleCheckout = () => {
      const selectedItems = cartItem.filter(item => selectedSet.has(String(item.id)));
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-8">
               ...
               <p className="text-gray-600">
                  {cartItem.length} {cartItem.length === 1 ? 'item' : 'items'} in your cart · {cartItem.filter(i => selectedSet.has(i.id)).length} selected
               </p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-md p-4 border border-gray-100 mb-8">
               ...
               <input 
                  type="checkbox"
                  checked={cartItem.filter(i => i.products?.stock).every(item => selectedSet.has(item.id))}
               ...
               <div className="lg:col-span-2 space-y-4">
                  {cartItem.length > 0 ? (
                     cartItem.map((item) => (
                        <div 
                           key={item.id} 
                           className={`bg-white rounded-2xl shadow-md hover:shadow-xl transition-all p-6 border ${selectedSet.has(item.id) ? 'border-blue-500' : 'border-gray-100'}`}
                        >
                              ...
                              <div className="flex-1 min-w-0">
                                 <div className="flex justify-between gap-4 mb-3">
                                    <div className="flex-1">
                                       <h3 className="text-xl font-bold text-gray-900 mb-2">{item.products?.name}</h3>
                                    ...
                                    <div className="text-right">
                                       <p className="text-2xl font-bold text-gray-900">${item.products?.price.toFixed(2)}</p>
                                 ...
                                 <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                                    ...
                                    <div className="flex items-center gap-4">
                                       ...
                                       {item.quantity >= (item.products?.stock || 0) && (
                                          <span className="text-xs text-orange-600 font-medium">Max stock reached</span>
                                       )}
                                    </div>
                     ...
                     <div className="bg-white rounded-2xl shadow-md p-16 text-center">
                        ...
                        <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-xl transition-all shadow-lg hover:shadow-xl">
                           Continue Shopping
                        </button>
                     </div>
                  )}
                     ...
                     <div className="p-6 space-y-6">
                        ...
                        <div>
                           ...
                           <div className="flex gap-2">
                              ...
                              <button 
                                 // onClick={applyCoupon}
                                 className="px-6 py-3 bg-gray-900 hover:bg-gray-800 text-white font-semibold rounded-xl transition-all"
                              >
                                 Apply
                              </button>
                           </div>
                        ...
                        <button 
                           ...
                           disabled={cartItem.filter(item => selectedSet.has(item.id)).length === 0}
                        ...
                        <div className="grid grid-cols-3 gap-3 pt-4 border-t border-gray-200">
                           ...
                           <div className="text-center">
                              ...
                              <p className="text-xs text-gray-600 font-medium">Safe Payment</p>
                           </div>
                        </div>
                     </div>
                  </div>
